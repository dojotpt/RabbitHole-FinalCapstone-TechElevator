...
export default {
  ...
  data() {
    return {
      // Since props are read-only, initialize a new object with the same property values.
      editTopic: {
  ...
  methods: {
    submitForm() {
      if (this.validateForm()) {
        CollectionService.update(this.editTopic)
          .then(() => {
    ...
    handleErrorResponse(error, verb) {
      if (error.response) {
        if (error.response.status === 404) {
          ...
          this.$store.commit('SET_NOTIFICATION', `Error ${verb} topic. Response received was "${error.response.statusText}".`);
        }
